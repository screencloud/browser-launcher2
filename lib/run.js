	...
	return function( uri, options, callback ) {
		...
		function run( customEnv ) {
			...
			setups[ browser.type ]( browser, uri, options, function( err, args ) {
				...
				var instance = new Instance( _.extend( {}, browser, {
					args: args,
				...
				function onError( code, error ) {
					callback( error.code === 'ENOENT' ? 'No matches for ' + name + '/' + version : error );
				}

				// report an instance's startup error
				instance.once( 'error', onError );

				instance.start();

				// in the next event loop we should know if the instance started successfully
				process.nextTick( function() {
					// instance has a PID - proceed
					if ( instance.pid ) {
						callback( null, instance );
						// we no longer need the startup error handler
